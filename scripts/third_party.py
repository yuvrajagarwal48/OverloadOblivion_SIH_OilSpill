...
async def analyze_oil_spill(ais_data: AISData):
    ...
    # Prepare query
    query = f"""Oil Spill Details:
    Location: Latitude {ais_data.Latitude}, Longitude {ais_data.Longitude}
    Affected Area: {ais_data.AffectedArea} square kilometers
    Vessel: {ais_data.VesselName} (IMO: {ais_data.IMO}, MMSI: {ais_data.MMSI})
    Environmental Control Area (ECA): {ais_data.ECA}
    Zone: {ais_data.Zone}
    
    Analyze environmental impacts, marine ecosystem effects, and provide specific predictions."""
